...
uint16_t lvl[FRAMES],           // Audio level for the prior #FRAMES frames
         ...
         avgHi  = 1024,          // Audio volume upper end of range
...
void loop() {
  ...
  //this was a hack to get the top 5 lights free
  ...
  //This is a thing that i did, it sucks

  coolstuff += sum;
  if(CircuitPlayground.leftButton()){
    coolstuff = 0;
  }
  if(coolstuff>=maxcoolstuff){
    coolstuff=maxcoolstuff;
  }
  placeholder = ((maxcoolstuff - coolstuff) / (maxcoolstuff/10));

  Serial.println(placeholder);
  Serial.println(coolstuff);
  for (i=5; i < placeholder+1; i++){
    CircuitPlayground.strip.setPixelColor(i, CircuitPlayground.strip.Color(0,0,0));
  }
  for (i = 10; i > placeholder; i--) {
    CircuitPlayground.strip.setPixelColor(i, CircuitPlayground.strip.Color(205, 127, 50));

  }
  CircuitPlayground.strip.setBrightness(30);
  CircuitPlayground.strip.show();
  Serial.println(sum);
  //This is where the suck stops
  peak += peakV;
  if (peak <= 0) {
    peak  = 0;
    peakV = 0;
