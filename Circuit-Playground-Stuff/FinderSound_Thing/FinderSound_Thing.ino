...
void loop() {
  ...
  for(i=0; i<BINS; i++) { // For each output bin (and each pixel)...
    ...
    Serial.println(lvl[frameIdx][5]);
    if (
      lvl[frameIdx][0]<15 and
      lvl[frameIdx][1]<15 and
      lvl[frameIdx][2]<15 and
      lvl[frameIdx][3]<15 and
      lvl[frameIdx][7]<15 and
      30<lvl[frameIdx][5] and lvl[frameIdx][5]<40){
    ...
    //CircuitPlayground.playTone(level,20);
